...
class BFE(nn.Module):
    def __init__(self, num_classes, width_ratio=0.5, height_ratio=0.5):
        ...
        self.global_softmax = nn.Conv2d(512, num_classes, 1)
        ...
        self.softmax1 = nn.Conv2d(1024, num_classes, 1)
    ...
    def forward(self, x, y=None):
        ...
        # part1 branch
        if self.training:
            global_x = self.global_reduction(x)  # .squeeze()
            global_maps = self.global_softmax(global_x)
            global_maps = global_maps.detach()
            # gradcam, _ = gradCAM(x, global_softmax_class, y)
            scores = []
            for i in range(x.size()[0]):
                scores.append(global_maps[i, y[i], :, :])
            scores = torch.stack(scores, 0)
            x_part1, idx_cutmix1, lamda = self.cutmix_batch_drop1(x_part1, scores, y, [3, 2], 3)
        ...
        triplet_features.append(cutmix1_feature.squeeze())
        softmax_features.append(cutmix1_softmax_feature.squeeze())
        predict.append(cutmix1_feature.squeeze())

        # part2 branch
        if self.training:
            cutmix1_x = self.reduction1(x_part1)  # N*1024
            cutmix1_maps = self.softmax1(cutmix1_x)  # N*num_class/751
            cutmix1_maps = cutmix1_maps.detach()
            scores = []
            for i in range(x.size()[0]):
                scores.append(cutmix1_maps[i, y[i], :, :])
            scores = torch.stack(scores, 0)
            # gradcam, _ = gradCAM(x_part1, cutmix1_softmax_feature, y)
            x_part2, idx_cutmix2, lamda = self.cutmix_batch_drop1(x_part2, scores, y, [3, 2], 3)
        cutmix2_triplet_feature = self.part_maxpool(x_part2)#.squeeze()  # N*2048
        cutmix2_feature = self.reduction1(cutmix2_triplet_feature)  # N*1024
        cutmix2_softmax_feature = self.softmax1(cutmix2_feature)  # N*num_class/751
        triplet_features.append(cutmix2_feature.squeeze())
        softmax_features.append(cutmix2_softmax_feature.squeeze())
        predict.append(cutmix2_feature.squeeze())
