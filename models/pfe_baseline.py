...
class BFE(nn.Module):
    def __init__(self, num_classes, width_ratio=0.5, height_ratio=0.5):
        ...
        self.global_softmax = nn.Linear(1024, num_classes, bias=False)
    ...
    def forward(self, x, y=None):
        ...
        # global branch
        glob = self.global_avgpool(x)#.squeeze()
        global_triplet_feature = self.global_reduction(glob).squeeze()
        global_triplet_feature = F.normalize(global_triplet_feature, dim=1, p=2)
