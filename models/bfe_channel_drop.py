...
class BFE(nn.Module):
    def __init__(self, num_classes, width_ratio=0.5, height_ratio=0.5):
        ...
        self.global_softmax_c = nn.Linear(512, num_classes) 
        self.global_softmax_c.apply(weights_init_kaiming)
        self.global_reduction_c = nn.Sequential(
            nn.Linear(768, 512), 
            nn.BatchNorm1d(512), 
            nn.ReLU()
        )
        self.global_reduction.apply(weights_init_kaiming)
        self.global_reduction_c.apply(weights_init_kaiming)
        ...
        #self.batch_drop_c = nn.Dropout2d(0.5)
    ...
    def forward(self, x):
        ...
        x_upsam = self.upsample(x)
        globc = x_upsam.mean(1)
        b,h,w = globc.size()
#        print(b,h,w)
        globc = globc.view(b, h*w)
        global_triplet_feature = self.global_reduction_c(globc)
        global_softmax_class = self.global_softmax_c(global_triplet_feature)
        softmax_features.append(global_softmax_class)
        triplet_features.append(global_triplet_feature)
        predict.append(global_triplet_feature)
    ...
    def get_optim_policy(self):
        params = [
            ...
            {'params': self.global_reduction_c.parameters()},
            {'params': self.global_softmax_c.parameters()},
