...
class BFE(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        patchs = patchs_row.sum(dim=[2, 3])  # N*12
        predicted_mask = self.softmax(patchs)
        predicted_mask_for_test = predicted_mask.detach()
        ...
        if self.training:
            ...
            patchs_row = torch.stack(cam_patchs_row, 1)
            patchs = patchs_row.sum(dim=[2, 3])  # N*12
            element, idx = patchs.sort(descending=True)
            # label_mask = torch.zeros_like(patchs)
            # for i in range(bs):
            #     label_mask[i, idx[i,0]] = 1
        ...
        patchs = patchs_row.sum(dim=[3, 4])  # N*12
