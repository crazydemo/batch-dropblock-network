...
class BFE(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        score_tensor.append(global_triplet_feature)
        ...
        if self.training:
            x_detached = x.detach()
            score = self.global_reduction(x_detached)  # .squeeze()
            score = self.global_softmax(score)
            gradcam = []
            for i in range(score.size()[0]):
                gradcam.append(score[i, y[i], :, :])
            gradcam = torch.stack(gradcam, 0)
            gradcam_detached = gradcam.detach()
            x_part1, idx_cutmix1, lamda = self.cutmix_batch_drop1(x_part1, gradcam_detached, y, [3, 2], 3)
        ...
        cutmix1_feature = self.reduction1(x_part1)  # N*1024
        score_tensor.append(cutmix1_feature)
	...
	if self.training:
        ...
        '''
        if self.training:
            return triplet_features, softmax_features, score_tensor
        else:
