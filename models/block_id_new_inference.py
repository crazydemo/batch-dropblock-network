...
class BFE(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        predicted_score = self.global_softmax(global_triplet_feature).squeeze()

        global_triplet_feature = self.global_reduction(x)
        global_softmax_class = self.global_softmax(global_triplet_feature)

        bs, c, h, w = global_softmax_class.size()
        global_softmax_class = global_softmax_class.view(bs, c, -1)
        score = self.softmax(global_softmax_class)
        score = score.view(bs, c, h, w)
        score_lst = []
        _, predicted_y = predicted_score.max(-1)
        for i in range(x.size()[0]):
            score_lst.append(score[i, predicted_y[i], :, :])
        score = torch.stack(score_lst, 0).unsqueeze(1)

        x *= score
        global_triplet_feature = self.global_reduction(x)
        global_softmax_class = self.global_softmax(global_triplet_feature)
        global_triplet_feature = torch.sum(global_triplet_feature, [2,3])
