...
class CutMixBatchChannelDrop(nn.Module):
    def __init__(self, p):
        super(CutMixBatchChannelDrop, self).__init__()
        self.p = 1-p

    def forward(self, x, cam, y, randy=None):
        '''
        :param x: feature map with shape[N,2048,24,8]
        :param cam: grad cam map with shape[N,24,8]
        :param y: gt of x, [N,1]
        ...
        if self.training:
            ...
            '''get mask'''
            _, f_c = cam.size()
            patchs, patchs_idxs = cam.sort(1, True)
            k = int(f_c * self.p)
            for i in range(bs):
                mask[i, pidx[i,:k], :, :] = 0
...
def gradCAMChannel(gradfeature, scores, idxs):
    getGrad = GetGrad()
    getGrad(gradfeature)
    feature = gradfeature.detach()
    bs, c, h, w = feature.size()
    ...
    cam_cv = []
    for i in range(bs):
        grad_cam = weight[i].reshape(1, c).T * feature[i].reshape(c, h * w)
        #grad_cam = grad_cam.reshape(h, w)
        grad_cam = F.relu(grad_cam)
        grad_cam = grad_cam.sum(-1).unsqueeze(0)
        output_gradCam.append(grad_cam)
        '''
        cam_img = grad_cam.cpu().detach().numpy()
        cam_img = cam_img - np.min(cam_img)
...
class BFE(nn.Module):
    def __init__(self, num_classes, width_ratio=0.5, height_ratio=0.5):
        ...
        # part11
        self.channel_drop11 = CutMixBatchChannelDrop(p=0.7)
        self.reduction11 = nn.Sequential(
            nn.Linear(2048, 1024, 1),
            nn.BatchNorm1d(1024),
            nn.ReLU()
        )
        self.reduction11.apply(weights_init_kaiming)
        self.softmax11 = nn.Linear(1024, num_classes)
        self.softmax11.apply(weights_init_kaiming)
        ...
        # part21
        self.channel_drop21 = CutMixBatchChannelDrop(p=0.7)
        self.reduction21 = nn.Sequential(
            nn.Linear(2048, 512, 1),
            nn.BatchNorm1d(512),
            nn.ReLU()
        )
        self.reduction21.apply(weights_init_kaiming)
        self.softmax21 = nn.Linear(512, num_classes)
        self.softmax21.apply(weights_init_kaiming)
    ...
    def forward(self, x, y=None):
        ...
        # part11 branch
        if self.training:
            gradcamchannel, _ = gradCAMChannel(x, global_softmax_class, y)
            x_part11 = self.channel_drop11(x_part11, gradcamchannel, y)
        ...
        triplet_features.append(cutmix1_feature+cp1_feature)
        softmax_features.append(cutmix1_softmax_feature+cp1_softmax_feature)
        predict.append(cutmix1_feature+cp1_feature)
    ...
    def get_optim_policy(self):
        params = [
            ...
            {'params': self.reduction11.parameters()},
            {'params': self.softmax11.parameters()},
            {'params': self.reduction21.parameters()},
            {'params': self.softmax21.parameters()},
        ]
        return params
