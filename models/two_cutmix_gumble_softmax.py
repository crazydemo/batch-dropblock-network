...
def gradCAM(gradfeature, scores, idxs):
    ...
    for i in range(bs):
        ...
        cam_img = cam_img / (np.max(cam_img)+1e-6)
...
class BFE(nn.Module):
    ...
    def forward(self, x, epoch=None, y=None):
        ...
        # part1 branch
        if self.training:
            gradcam, cam_vis = gradCAM(x, global_softmax_class, y)
            gradcam_ = torch.tensor(cam_vis).cuda().unsqueeze(1)#gradcam.detach().unsqueeze(1)
            score = gradcam_.sum(-1)
            scores = []
            for i in range(score.size()[-1]-2):
                scores.append(score[:, :, i*3:(i+1)*3].sum([0,1]))
            scores = torch.cat(scores, -1)
            sorted_scores, idx = torch.sort(scores, dim=-1, descending=True)
            ...
            gradcam_ = self.sigmoid(gradcam)
            mask1 = self.gumble1(gradcam_, epoch)
            mask1[:, [0], :, :] = mask[:, [0], :, :] * mask1[:, [0], :, :]
            # mask1 = mask1[:, [1], :]
            # mask1 = mask1.unsqueeze(-1)
            # mask1 = mask1.repeat(1, 1, 1, 8)
            # mask1 = mask1.view(-1, 2, 24, 8)
            x_part1 = x_part1 * mask1[:, [0], :, :]
            # x_part1 = mask * x_part1
        ...
        # part2 branch
        if self.training:
            gradcam, cam_vis = gradCAM(x_part1, cutmix1_softmax_feature, y)
            gradcam_ = torch.tensor(cam_vis).cuda().unsqueeze(1)  # gradcam.detach().unsqueeze(1)
            score = gradcam_.sum(-1)
            scores = []
            for i in range(score.size()[-1] - 2):
            ...
            gradcam_ = self.sigmoid(gradcam)
            mask2 = self.gumble2(gradcam_, epoch)
            mask2[:, [0], :, :] = mask[:, [0], :, :] * mask2[:, [0], :, :]
            # mask1 = mask1[:, [1], :]
            # mask1 = mask1.unsqueeze(-1)
            # mask1 = mask1.repeat(1, 1, 1, 8)
            # mask1 = mask1.view(-1, 2, 24, 8)
            x_part2 = x_part2 * mask2[:, [0], :, :]
            # x_part2 = mask * x_part1
            # x_part1, idx_cutmix1, lamda = self.cutmix_batch_drop1(x_part1, gradcam, y, [3, 2], 3)
        ...
        if self.training:
            return triplet_features, softmax_features, mask1, cam_vis.squeeze()
        else:
