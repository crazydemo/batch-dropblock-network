...
def gradCAM(gradfeature, scores, idxs):
    ...
    '''normalize'''
    # grad /= (1e-12 + torch.max(torch.abs(grad), keepdim=True))
    grad /= torch.sqrt(1e-6 + torch.sum(torch.pow(grad, 2.0), [1,2,3], keepdim=True))
...
class BFE(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        # part1 branch
        if self.training:
            mask = gradCAM(x, global_softmax_class, y)
            x_part1 = x_part1 + 16*mask
