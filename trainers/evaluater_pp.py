...
class ResNetEvaluator:
    ...
    def evaluate(self, queryloader, galleryloader, queryFliploader, galleryFliploader,
        ...
        q_g_dist1 = torch.pow(qf[:,:512], 2).sum(dim=1, keepdim=True).expand(m, n) + \
                   torch.pow(gf[:,:512], 2).sum(dim=1, keepdim=True).expand(n, m).t()
        q_g_dist1.addmm_(1, -2, qf[:,:512], gf[:,:512].t())
        q_g_dist2 = torch.pow(qf[:,512:], 2).sum(dim=1, keepdim=True).expand(m, n) + \
                   torch.pow(gf[:,512:], 2).sum(dim=1, keepdim=True).expand(n, m).t()
        q_g_dist2.addmm_(1, -2, qf[:,512:], gf[:,512:].t())
